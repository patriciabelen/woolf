...
$('#sentences li').on('click', function () {
...
function animateDiv(){
    ...
    var oldq = $('.snail').offset();
    var speed = calcSpeed([oldq.top, oldq.left], newq);
    
    $('.snail').animate({ top: newq[0], left: newq[1] }, speed, function(){
      animateDiv();        
    });
...
function animateCirc(){
    ...
    var speed = calcSpeedb([oldq.top, oldq.left], newq);
...
gsap.set('.cards li', {xPercent: 450, opacity: 0, scale: 0});
...
const spacing = 0.15, // spacing of the cards (stagger)
	...
	animateFunc = element => {
		...
		tl.fromTo(element, {scale: 0, opacity: 0}, {scale: 2.2, opacity: 1, zIndex: 100, duration: 0.5, yoyo: true, repeat: 1, ease: "power1.in", immediateRender: false})
		  .fromTo(element, {xPercent: 450}, {xPercent: -400, duration: 1, ease: "none", immediateRender: false}, 0);
		return tl;
	},
